...
from openerp.addons.ept_rendicion.controladores.ept_rnd_rendicion_c import rendicion
...
class ept_ent_entidades(osv.osv):
    _name = 'ept_ent.entidades'
    ...
    def __init__(self, pool, cr):
        init_res = super(ept_ent_entidades, self).__init__(pool, cr)
        ...
        if arreg:
            if len(usuarios_ids)==1:
                ...
                cr.execute('SELECT rel.entidad_id,rel.entidad_user_id '\
                            'FROM ept_ent_entidad_user_rel AS rel '\
    ...
    def _get_attachment_number(self, cr, uid, ids, fields, args, context=None):
        ...
        for app_id in ids:
            res[app_id] = self.pool['ir.attachment'].search_count(
                                            cr,
                                            uid,
                                            [('res_model', '=', 'ept_ent.entidades'), ('res_id', '=', app_id)],
                                            context=context)
        return res
    ...
    def _get_comunicaciones_number(self, cr, uid, ids, fields, args, context=None):
        ...
        for app_id in ids:
            ...
            res[app_id] = self.pool['ept_com.comunicaciones'].search_count(
                                            cr,
                                            uid,
    ...
    def _get_comunicaciones_cfg_number(self, cr, uid, ids, fields, args, context=None):
        ...
        for app_id in ids:
            entidad_data=self.browse(cr,uid,[app_id])[0]
            res[app_id] = self.pool['ept_com.comunicaciones_masivas'].search_count(
                                            cr,
                                            uid,
                                            [
    ...
    def _get_comunicaciones_fcg_enviada_number(self, cr, uid, ids, fields, args, context=None):
        ...
        for app_id in ids:
            entidad_data=self.browse(cr,uid,[app_id])[0]
            comunicaciones_obj=self.pool['ept_com.comunicaciones_masivas']
    ...
    _columns = {
        ...
        'municipio_ids':fields.many2many(
                    'ept_ent.municipios',
                    'ept_ent_relacion_ent_municipios',
                    'entidad_id',
                    'municipios_id',
                    'Relacion Municipios'
                    ),
        ...
        'parroquia_ids':fields.many2many(
                    'ept_ent.parroquias',
                    'ept_ent_relacion_ent_parroquias',
                    'entidad_id',
                    'parroquias_id',
                    'Relacion Parroquias'
                    ),
    ...
    def action_get_attachment_tree_view(self, cr, uid, ids, context=None):
        ...
        return action
    #~ inicio acciones de las comunicaciones de las EPT al FCI
    def action_get_comunicaciones_tree_view(self, cr, uid, ids, context=None):
        model, action_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'ept_comunicaciones', 'ept_com_comunicaciones_action')
        action = self.pool.get(model).read(cr, uid, action_id, context=context)
    ...
    def action_get_comunicacionesProceso_tree_view(self, cr, uid, ids, context=None):
        ...
        return action
    #~ fin de las aciones de las comunicaciones de las EPT al FCI
    #~ inicio de las acciones de las comunicaciones del FCI a las EPT
    
    def action_get_comunicacionesMasivas_tree_view(self, cr, uid, ids, context=None):
        model, action_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'ept_comunicaciones', 'ept_com_comunicaciones_masivas_action')
        action = self.pool.get(model).read(cr, uid, action_id, context=context)
        entidad_data=self.browse(cr,uid,ids)[0]
    ...
    def action_get_comunicacionesMasivasLeidas_tree_view(self, cr, uid, ids, context=None):
     ...
     #~ fin de las acciones de las comunicaciones del FCI a las EPT
    ...
    def create(self,cr,uid,values,context=None):
        ...
        return super(ept_ent_entidades,self).create(
                                                    cr,
                                                    uid,
                                                    values,
                                                    context=context)
    ...
    def write(self, vals):
        vals['editar']=False
        entidad_id = super(ept_ent_entidades, self).write(vals)
        if(vals.has_key('image1')):
            movil_users_obj=self.pool('ept_movil.movil_users')
