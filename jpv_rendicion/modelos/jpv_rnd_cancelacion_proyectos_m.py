...
class jpv_rnd_cancelacion_proyecto(osv.osv):
    _name='jpv_rnd.cancelacion_proyecto'
    _columns = {
        ...
        'movimiento_id':fields.many2one('jpv.movimientos_cuentas','Movimiento')
    }


    def create(self, cr, uid, values, context=None):
        proyecto_data=self.pool.get('jpv_cp.carga_proyecto').browse(cr,uid,values['proyecto_id'])
        ...
        if not proyecto_data.state == 'cancelado':
            write_state_proyecto=self.pool.get('jpv_cp.carga_proyecto').write(cr,
                                                                uid,
                                                                values['proyecto_id'],
                                                                {'state':'cancelado'},0)

            cancel_id=super(jpv_rnd_cancelacion_proyecto,self).create(cr,uid,values)
            ...
            obj_mov_cuentas=self.pool.get('jpv.movimientos_cuentas')
            ...
            abono_a_cuenta_id=obj_mov_cuentas.movimiento_ingreso(cr,uid,cuenta_id,
                                                                        ...
                                                                        'jpv_cp.carga_proyecto',cancel_data.proyecto_id.id,
            ...
            if cancel_data.proyecto_id.proyect_mantenimiento:
                                ...
                                write_monto_mantenimiento=self.pool.get('jpv_cp.carga_proyecto').write(cr,
                                                                    SUPERUSER_ID,
            ...
            if write_state_proyecto:
                historial_proyecto_obj=self.pool.get('jpv_cp.historial_proyecto')
                historial={
    ...
    def registro_solicitud_firma_carta_cancelacion_proyecto(self,cr,uid,proyecto,cancel_data):
        ...
        list_datos_adicionales.append([0,False,{'objeto_ratro':'jpv_cp.carga_proyecto', 'objeto_ratro_id':proyecto.id, 'referncia':referencia,}])
        value_x_firmar={
            ...
            'objeto_ratro_principal':'jpv_rnd.cancelacion_proyecto',
    ...
    def crear_carta_cancelacion_proyecto(self,cr,uid,cartaxfirmar_data,context=None):
        reportname='jpv_rendicion.cancelacion_proyecto_qweb'
    ...
    def carga_carta_cancelacion_proyecto(self,cr,uid,carta_data,file_name,file_data,context):
        ...
        for i in carta_data:
            ...
            cancelacion_id=self.pool.get('jpv_rnd.cancelacion_proyecto').search(cr,uid,[('id','=',cancel_id)])
            if not len(cancelacion_id):
                cancelacion_id=self.pool.get('jpv_rnd.cancelacion_proyecto').search(cr,uid,[('proyecto_id','=',cancel_id)])
            ...
            cancel_data=self.pool.get('jpv_rnd.cancelacion_proyecto').browse(cr,uid,cancel_id)
        ...
        attachment_id = ir_attachment_obj.create(cr,uid,{
            ...
            'res_model':'jpv_cp.carga_proyecto',
