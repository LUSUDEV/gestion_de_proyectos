...
class jpv_rnd_avance(osv.osv):
    _name='jpv_rnd.avance'
    ...
    _columns = {
        ...
        'proyecto_id': fields.related('rendicion_id','proyecto_id',string='Proyecto', 
                        type='many2one', relation='jpv_cp.carga_proyecto',
                        help="""Entidad a la cual esta asociada el proyecto"""),
                    
                    
        'respuestas_line':fields.one2many('jpv_rnd.input_line','avance_id','Respuestas'),
    }
    ...
    def unlink(self,cr,uid,ids,context=None):
        ...
        for avance in avance_data:
            if avance.rendicion_id.state=='culminada':
                ...
                print self
                unlink_id=super(jpv_rnd_avance,self).unlink(cr,uid,ids,context)
        return unlink_id 

    
class jpv_rnd_input_line(osv.osv):
    _name="jpv_rnd.input_line"
    
    
    
    _columns = {
    
        'pregunta_id' : fields.many2one('jpv_conf.preguntas', 'Pregunta',ondelete="restrict"),
       
        'avance_id' : fields.many2one('jpv_rnd.avance', 'Avance',ondelete="cascade"),
        
        'rendicion_id' : fields.related('avance_id', 'rendicion_id', 
                            string='Rendicion', type='many2one', 
                            relation='jpv_rnd.avance', help="""Rendicion"""),
        
        'tipo_pregunta': fields.related('pregunta_id','tipo',string='tipo de pregunta', 
                        type='char', relation='jpv_conf.preguntas',
...
class ir_attachment_inherit(osv.osv):
    ...
    _columns = {
        ...
        'input_line_id' : fields.many2one('jpv_rnd.input_line', 'Respuesta', ondelete="cascade"),
