...
class res_users(osv.osv):
    ...
    def raise_groups_id(self, cr, uid, context=None):
        ...
        for groups in groups_data:
            grupos+=groups.name+' \n'
        raise osv.except_osv(('Error de asignación de grupo'),
    ...
    def default_action(self,cr,uid,name_action,context=None):
        ir_action_obj=self.pool.get('ir.actions.actions')
        ir_action_ids=ir_action_obj.search(cr,uid,[('name','=',name_action)])
        return ir_action_ids[0]
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if context.has_key('action_name'):
            name_action=context['action_name']
            vals['action_id']=self.default_action(cr,uid,name_action)
