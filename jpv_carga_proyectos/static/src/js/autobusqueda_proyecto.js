$(document).ready(function(){
    ...
    $.each( $.find('#jpv_busqueda_avanzada_pro'),function(){
        openerp.jsonRpc('/jpv_cp_proyecto_consulta/busqueda_municipios', 'call', {}).then(function (data_municipio) {
            ...
            $('select#municipio_bq_avz_id').on('change', function(event){
                    ...
                    openerp.jsonRpc("/jpv_cp_proyecto_consulta/busqueda_parroquias", 'call', {
...
$('input.jpv_date_buz_proy').mask("00-00-0000");
    $('input.jpv_date_buz_proy').datepicker({
            dateFormat: "dd-mm-yy"
            });
    ...
    $("#desplega_busq_avzda").click(function(){
     ...
     $('#jpv_limpia_campos_proy').click(function(){
    ...
    $('.js_buscar').live( "click", function() {
        ...
        $('#jpv_buscador_proyecto').val(name)
        openerp.jsonRpc('/jpv_cp_proyecto_consulta/seleccion', 'call', {'id':id}).then(function (respuesta) {
    ...
    $( "#jpv_buscador_proyecto" ).keyup(function(event) {
            $('.js_busqueda_avanzada_proy').hide();
            $('#buscando').html("<p align='center' >\
                                <img width='25px' \
                                height='25px' \
                                src='jpv_carga_proyectos/static/src/img/ajax-loader.gif' />\
                                </p>");
            var value=$(this).val();
            if (value!=''){
            openerp.jsonRpc('/jpv_cp_proyecto_consulta', 'call', {'key':value,'code':event.keyCode}).then(function (respuesta) {
    ...
    $( "#busca_avanzada" ).click(function() {
         var form='#jpv_busqueda_avanzada_pro';
