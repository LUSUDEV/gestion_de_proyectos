...
from openerp.addons.ept_usuarios.controladores.ept_use_users_c import cfg_users as ept_usuarios
#~ from openerp.addons.ept_carga_proyectos.controladores import ept_carga_proyecto_c as ecp
import comunes
import json
...
class rendicion(http.Controller):
    ...
    def activar_carga(self,proyecto_id,uid):
        ...
        proyecto_obj=registry['ept_cp.carga_proyecto']
        proyecto_write=proyecto_obj.write(cr,SUPERUSER_ID,proyecto_id,{'avance':True},0)
        proyecto=self.instanciar_objetos('ept_cp.carga_proyecto',proyecto_id)
        proyectos_data=self.instanciar_objetos('ept_cp.carga_proyecto',
                                                None,
                                                [('partner_id','=',proyecto.partner_id.id),
                                                ('state','=','aprobado'),
                                                ('avance','=',False)])
        if not len(proyectos_data):
            entidad_data=self.instanciar_objetos('ept_ent.entidades',
                                                None,
                                                [('parent_id','=',proyecto.partner_id.id)])
            write_entidad=registry['ept_ent.entidades'].write(cr, SUPERUSER_ID,
                                                entidad_data.id,
    ...
    def ocultar_boton_crear(self,partner_id):
        ...
        hoy=date.today()
        format="%Y"
        periodos_obj = registry['ept_plf.periodos']
        anio=hoy.strftime(format)
        actividad="CARGA DE PROYECTOS"
        entidades_data=self.instanciar_objetos('ept_ent.entidades',None,[('parent_id','=',int(partner_id))])
    ...
    def rendicion_proyecto_get(self,proyecto_id):
        redicion_data=self.instanciar_objetos('ept_rnd.rendicion', None,[('proyecto_id','=',proyecto_id)])
    ...
    def tiempo_validez_get(self,proyecto_id,uid=None):
        ...
        hoy=datetime.today()
        retorno='success'
        entidad_data=ept_usuarios().mi_entidad(uid)
        proyecto_data=self.instanciar_objetos('ept_cp.carga_proyecto', proyecto_id)
        tiempo_data=self.instanciar_objetos('ept_rnd.tiempo_validez', None,[('model_id','=','ept_rnd.rendicion')])
        ...
        #planificacion_data=registry['ept_plf.periodos'].plf_control_actividades(cr,SUPERUSER_ID,[],entidad_data['entidad_data'].parent_id.id,actividad)
        
        if not len(redicion_data):
            # if not 'periodo' in planificacion_data:
            self.deshabilitar_carga(proyecto_id)
            return 'danger'
        avance_data=self.instanciar_objetos('ept_rnd.avance', None,[('rendicion_id','=',redicion_data.id)],'create_date DESC',1)
    ...
    def deshabilitar_carga(self,proyecto_id):
        ...
        context = http.request.context
        proyecto=self.instanciar_objetos('ept_cp.carga_proyecto',proyecto_id)
        proyecto_write=registry['ept_cp.carga_proyecto'].write(cr,SUPERUSER_ID,proyecto_id,{'avance':False},0)
        entidad_data=self.instanciar_objetos('ept_ent.entidades',
                                                None,
                                                [('parent_id','=',proyecto.partner_id.id)])
        write_entidad=registry['ept_ent.entidades'].write(cr, SUPERUSER_ID,
    ...
    def ocultar_proyecto_editado(self,proyecto_id):
        proyecto_data=self.instanciar_objetos('ept_mig.proyectos_migrados',None,[('proyectos_id','=',int(proyecto_id)),('actualizado','=',True)])
    ...
    @http.route(['/ept_mostrar_proyectos'], type='json', auth='public', website=True)
    ...
    def busca_respuestas(self, datos_met=None, **datos):
        ...
        preguntas_ms_ids=[]
        preguntas_mc_ids=[]
        input_line_obj=registry['ept_rnd.input_line']
    ...
    def avance(self, id_proyecto):
        ...
        entidad_data=ept_usuarios().mi_entidad(uid)
        responsable=[]
        proyecto_migrado_obj=registry['ept_mig.proyectos_migrados']
        proyecto_migrado_id=proyecto_migrado_obj.search(cr,SUPERUSER_ID,[('proyectos_id','=', id_proyecto)])
        if len(proyecto_migrado_id):
            proyecto_migrado_data=proyecto_migrado_obj.browse(cr,SUPERUSER_ID,proyecto_migrado_id)
            if not proyecto_migrado_data.actualizado:
                mensaje={
                    'titulo':'Proyecto sin actualizar',
                    'mensaje':'''Disculpe debe actualizar los datos de este proyecto antes de realizar un avance''',
                    'volver':'/'
                }
                return http.request.website.render('website_apiform.mensaje', mensaje)
        rendicion_question_obj=registry['ept_conf.obj_rendicion']
        ...
        datos.update({'parametros':{
                            ...
                            'template':'ept_rendicion.avance',
                            'css':'success',
                            'id_enviar':'id_guardar_rendicion_ept',
    ...
    def avance_consulta(self, id_proyecto):
        ...
        datos.update({'parametros':{
                            ...
                            'template':'ept_rendicion.consulta',
                            'css':'info',
    ...
    def avance_enviar(self, **kwargs):
        registry = http.request.registry
        cr, uid, context = http.request.cr, http.request.uid, http.request.context
        pregunta_obj=registry['ept_conf.preguntas']
            ...
            if i.split('-')[1] == 'file' or i.split('-')[1] == 'img':
                ...
                for campo in campos_adquisicion.keys():
                    ...
                    if not campo.split('-')[3] == 'id' and not campo.split('-')[3] == 'accion':
                        ...
                        descripcion_adquisicion=registry['ept_cp.carga_proyecto'].fields_get(cr,SUPERUSER_ID,nombre_adquisicion)[nombre_adquisicion]['string']
                ...
                if pregunta_data.field_muestra_ids and pregunta_data.field_muestra_ids.name=='monto_gastado':
                    ...
                    if len(rendicion_id):
                        ...
                        proyecto_data=registry['ept_cp.carga_proyecto'].browse(cr,SUPERUSER_ID, int(kwargs['proyecto_id']))
            ...
            if len(rendicion_id):
                ...
                if kwargs['declarar_culminacion'] == 'True':
                    ...
                    if rend_data.state == 'ejecucion':
                        ...
                        write_state_project=registry['ept_cp.carga_proyecto'].write(cr,
                                                                SUPERUSER_ID,
                        ...
                        if monto_gastado < rend_data.monto:
                            ...
                            obj_mov_cuentas=registry['ept.movimientos_cuentas']
                            ...
                            abono_a_cuenta_id=obj_mov_cuentas.movimiento_ingreso(cr,SUPERUSER_ID,cuenta_id,
                                                                        ...
                                                                        'ept_cp.carga_proyecto',rend_data.proyecto_id.id,
                            ...
                            if rend_data.proyecto_id.proyect_mantenimiento:
                                ...
                                write_monto_mantenimiento=registry['ept_cp.carga_proyecto'].write(cr,
                                                                    SUPERUSER_ID,
    ...
    def cancelar_proyecto(self, **kwargs):
        ...
        errors=panel.validar().varios_campos(campos,kwargs)
        if not len(errors):
            cancelacion_id=registry['ept_rnd.cancelacion_proyecto'].create(cr,
                                                            SUPERUSER_ID,
                                                            {'proyecto_id':int(kwargs['proyecto_id']),
    ...
    def descargar_reporte_ejecucion(self,id_proyecto, **post):
        ...
        registry = http.request.registry
        reportname='ept_rendicion.id_template_reporte_ejecucion_qweb_website'
        entidad_data=ept_usuarios().mi_entidad(uid)
        nombre=''
        rendicion_obj=registry['ept_rnd.rendicion']
        rendicion_id=rendicion_obj.search(cr,SUPERUSER_ID,[('proyecto_id','=',id_proyecto)])
    ...
    @http.route(['/eptProyectosVerMas'],type='json', auth='user', website=True)
    def ept_ver_mas_proyectos(self,**datos):
        cr, uid, context = request.cr, request.uid, request.context
        registry = http.request.registry
        inicio=datos['fin']
        fin=inicio+datos['page_limit']
        proyectos_data=registry['ept_cp.carga_proyecto'].read(cr,uid,datos['lista_proyectos_ids'][inicio:fin])
    ...
    @http.route(['/eptProyectosVerRendir'],type='json', auth='user', website=True)
    def ept_ver_proyectos_rendir(self,**datos):
        cr, uid, context = request.cr, request.uid, request.context
        registry = http.request.registry
        inicio=datos['num']
        fin=datos['fin']
    ...
    @http.route(['/eptProyectosxRendirListaEstadistica'],type='json', auth='user', website=True)    
    ...
    @http.route(['/eptProyectosxRendirEstadistica'],type='json', auth='user', website=True)    
