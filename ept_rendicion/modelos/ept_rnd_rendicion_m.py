...
from openerp.addons.ept_rendicion.controladores import comunes
from openerp import SUPERUSER_ID

class ept_rnd_rendicion(osv.osv):
    _name='ept_rnd.rendicion'
    ...
    def get_data_field(self, cr, uid, ids, nombre_campo, arg, context):
        ...
        pregunta_id=self.pool.get('ept_conf.preguntas').search(cr, uid, [('field_muestra_ids','=',campo_id[0])])
    ...
    def generar_notificaciones(self, cr, uid, ids=None, context=None):
        self.pool.get('ept_rnd.notificaciones_rendicion').consulta_rendiciones(cr,uid)
        return True
    ...
    def tiempo_validez_notificacion_get(self, cr, uid, proyecto_id):
        redicion_data=self.instanciar_objetos(cr, uid, 'ept_rnd.rendicion', None,[('proyecto_id','=',proyecto_id)])
        if not len(redicion_data):
            return 'sin_rendicion'
        avance_data=self.instanciar_objetos(cr, uid, 'ept_rnd.avance', None,[('rendicion_id','=',redicion_data.id)],'create_date DESC',1)
        tiempo_data=self.instanciar_objetos(cr, uid, 'ept_rnd.tiempo_validez', None,[('model_id','=',redicion_data._name)])
        hoy=datetime.today()
    ...
    _columns = {
        ...
        'adquisicion_equipos':fields.one2many('ept_cp.equipos','rendicion_id',
                                            string="Adquisicion de Equipos", 
                                            help="Aqui se agregan los equipos a adquirir"),
                                            
        'adquisicion_vehiculo':fields.one2many('ept_cp.vehiculo','rendicion_id',
                                            string="Adquisicion de Vehiculos", 
                                            help="Aqui se agregan los Vehiculos a adquirir"),
                                            
        'adquisicion_materiales_consumo':fields.one2many('ept_cp.materiales_consumo','rendicion_id',
                                            string="Adquisicion de Materiales de Consumo", 
                                            help="Aqui se agregan los materiales de consumo a adquirir"),
                                            
        'adquisicion_maquinaria':fields.one2many('ept_cp.maquinaria','rendicion_id',
                                            string="Adquisicion de Maquinaria", 
                                            help="Aqui se agregan la Maquinaria a adquirir"),
                                            
                                            
        'adquisicion_semovientes':fields.one2many('ept_cp.semovientes_caracteristicas','rendicion_id',
        ...
        'unidad_medicion_obra':fields.many2one('ept_cp.unidades_obra_civil', 
                                                'Unidad', 
                                                required=False, 
                                                help=''),
    ...
    def revertir_culminacion(self, cr, uid, ids, context):
        ...
        if rendicion_data.state == 'culminada':
            ...
            rendicion_data=self.browse(cr,uid,ids)
            disp_mantenimiento_entidad=self.pool.get('ept_ent.entidades').search_read(
                                                                            cr,
                                                                            uid,
                                                                            [('parent_id',
            ...
            if len(movimientos_lista):
                ...
                obj_movimiento=self.pool.get('ept.movimientos_cuentas')
                ...
                reverso_culminacion=obj_movimiento.movimiento_egreso(cr,SUPERUSER_ID,cuenta_id,
                                                                    ...
                                                                    'ept_cp.carga_proyecto',rendicion_data.proyecto_id.id,
                ...
                if rendicion_data.proyecto_id.proyect_mantenimiento:
                    ...
                    self.pool.get('ept_cp.carga_proyecto').write(cr,SUPERUSER_ID,
    ...
    def unlink(self,cr,uid,ids,context=None):
        ...
        for rendicion in rendicion_data:
            if rendicion.state=='culminada':
                ...
                unlink_id=super(ept_rnd_rendicion,self).unlink(cr,uid,ids,context)
        return unlink_id 

        
        
class ept_cp_equipos_inherit(osv.osv):
    _inherit="ept_cp.equipos"
...
class ept_cp_vehiculo_inherit(osv.osv):
    _inherit="ept_cp.vehiculo"
...
class ept_cp_materiales_consumo_inherit(osv.osv):
    _inherit="ept_cp.materiales_consumo"
...
class ept_cp_maquinaria_inherit(osv.osv):
    _inherit="ept_cp.maquinaria"
...
class ept_cp_semovientes_inherit(osv.osv):
    _inherit="ept_cp.semovientes_caracteristicas"
