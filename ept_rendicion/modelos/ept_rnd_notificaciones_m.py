...
class ept_rnd_notificaciones_rendicion(osv.osv):
    _name='ept_rnd.notificaciones_rendicion'
    ...
    _columns = {
        ...
        'rendiciones_actualizadas':fields.integer('Rendiciones Actualizadas'),
        'proyectos_sin_rendir':fields.integer('Proyectos Sin Rendir'),
                    
    }
    
    def consulta_rendiciones(self,cr,uid):
        rendicion_obj=self.pool.get('ept_rnd.rendicion')
        ...
        for entidad in entidades_data:
            cr.execute("select eccp.id from ept_cp_carga_proyecto as eccp inner join ept_ent_entidades as ee on ee.parent_id=eccp.partner_id"\
            ...
            vals={
                ...
                'rendiciones_actualizadas': len(values['actualizadas']),
            ...
            if vals['rendiciones_actualizadas'] > 0:
                string+=("%s rendiciones vigentes" % (vals['rendiciones_actualizadas']))
